...
- **머니플로우 핵심 (정본: `docs/05`)**: 자금은 **앱 포인트 기반 기간 제한형 에스크로**로 흐른다 — 예치(OPEN) → 만남시각 락(LOCKED) → 총무 출금(WITHDRAWN) → 증빙·잔액 반납(SETTLING) → 자동 균등 정산·현금화(DONE). **카카오페이는 포인트 충전·현금화 레일로만** 사용(회원 간 송금 없음). 잔액은 **포인트 원장(LedgerEntry)** 분개로 표현.
...
3. **총무 출금(WITHDRAWN)**: `POST /meetings/{id}/payout` — LOCKED 이후만. 본인 계좌로 현금화 + 정산 데드라인 타이머(예: 12h) 시작.
