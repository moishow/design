...
> 정본 문서. 토큰 값은 [`tokens.css`](tokens.css)가 단일 출처(SoT)이고, 디자인 기준화면은 [`meeting-detail.html`](meeting-detail.html)이다.
...
- 기준화면([`meeting-detail.html`](meeting-detail.html))이 살아있는 예시다: 루트는 `.surface-money`, 상단 모임 정체성 행만 코랄을 국소 허용.
...
구버전 토큰(`design/tokens/*.css`, `design/design-tokens.md`)은 참고용으로 남겨두되, **신규 작업은 `design/system/`을 따른다.**
...
[`mascots.svg`](mascots.svg) — flat SVG 젤리 4종(블루 주연 / 코랄 모임 / 민트 완료 / 옐로 장식) + 잉크 말풍선. **래스터·이모지 금지, 단순 stroke 표정.** 정식(production)은 이 벡터를 쓴다.

레퍼런스 3D 원본 캐릭터의 개별 추출 PNG(투명 배경)는 [`assets/mascots/`](assets/mascots/)에 보관(`mascot_blue/green/red/yellow/pink.png` + `speech-bubble.png`, 단체 `mascots.png`). 출처/제작용 참고 에셋이며 제품엔 직접 넣지 않는다.
...
| [`meeting-detail.html`](meeting-detail.html) | 디자인 기준화면(baseline) · money |
| [`onboarding.html`](onboarding.html) | community + 마스코트 + SNS 로그인 |
| [`home.html`](home.html) | 홈/리스트 + 탭바 |
| [`create-meeting.html`](create-meeting.html) | 폼(input·toggle·checkbox·select) |
| [`empty-state.html`](empty-state.html) | 빈 상태(마스코트 모먼트) |
| [`settlement-complete.html`](settlement-complete.html) | 정산 완료 축하 |
...
| [`mascots.svg`](mascots.svg) | 젤리 마스코트 벡터(정식) |
| [`assets/mascots/`](assets/mascots/) | 마스코트 원본 추출 PNG |
| [`flutter/moisho_theme.dart`](flutter/moisho_theme.dart) | Flutter ThemeData 매핑 |
| [`flutter/moisho_components.dart`](flutter/moisho_components.dart) | Flutter 위젯 카탈로그 |
